...
local function createNilPlacement()
	return setmetatable({}, {
		__index = function(self, index) error(`Expected identifier when parsing expression, got 'nil'`, 0) end,
		__newindex = function(self, index, key) error(`Expected identifier when parsing expression, got 'nil'`, 0) end,
...
local function createServerService() : Folder?
	if not use_env then
		if RunService:IsClient() then
			error('Cannot run on Client! (Please run it from the Server instead!)', 0)
			return nil
		end

		if not ServerStorage:FindFirstChild('Services') then
			local services = Instance.new('Folder', ServerStorage)
			services.Name = 'Services'
		end

		return ServerStorage:FindFirstChild('Services', true)
	else
		return env_folder
	end
end
task.spawn(createServerService)
...
return {
	...
	createService = function(instance : Instance, className : string, extra : {[string]: (...any) -> (...any)|{readonly: boolean?, value: any, property: boolean?}}, _self : {}?) : Service<{}>
		local serviceFolder = nil -- createServerService()
